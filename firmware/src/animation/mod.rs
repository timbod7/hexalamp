...
pub fn phase_color(
    ...
    let period2: usize = period / 2;

    let phase = framei % period;

    let (hue, val) = if phase < period / 2 {
        (
            linterp(phase as i32, 0, period2 as i32, bghue as i32, fghue as i32),
            linterp(phase as i32, 0, period2 as i32, bgval as i32, fgval as i32),
        )
    } else {
        (
            linterp(
